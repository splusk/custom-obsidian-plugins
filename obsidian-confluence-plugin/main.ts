...
export default class ConfluenceSyncPlugin extends Plugin {
	...
	async syncCurrentFile() {
		...
		try {
			...
			const page = await this.uploadToConfluence(title, contentToUpload, folderPath);
	...
	parseFrontmatter(content: string): { frontmatter: Map<string, string>; body: string; hasFrontmatter: boolean } {
	...
	updateFrontmatter(content: string, confluenceUrl: string): string {
		...
		// Add or update the confluence URL
		frontmatter.set('confluence', confluenceUrl);

		// Rebuild the frontmatter section
		const frontmatterLines = Array.from(frontmatter.entries()).map(([key, value]) => `${key}: ${value}`);
		const frontmatterText = frontmatterLines.join('\n');

		return `---\n${frontmatterText}\n---\n${body}`;
	}

	async uploadToConfluence(title: string, markdownContent: string, folderPath: string[]): Promise<ConfluencePage> {
		...
		let processedMarkdown = markdownContent.replace(/```mermaid\n([\s\S]+?)```/g, (_match, code) => {
			const filename = `MERMAID-PLACEHOLDER-${mermaidBlocks.length}.svg`;
			mermaidBlocks.push({ id: `MERMAID-PLACEHOLDER-${mermaidBlocks.length}`, filename, code: code.trim() });
			return `!${filename}`;
		});

		// Extract image attachments and replace with placeholders
		const imageAttachments: { filename: string; placeholder: string }[] = [];
		processedMarkdown = processedMarkdown.replace(/!\[\[([^\]]+\.(png|jpe?g))\]\]/gi, (_match, filename) => {
			const placeholder = `IMAGE-ATTACHMENT-${imageAttachments.length}`;
			imageAttachments.push({ filename: filename.trim(), placeholder });
			return placeholder;
		});

		const confluenceContent = this.convertMarkdownToConfluence(processedMarkdown);
		...
		const existingPage = await this.findExistingPage(title, auth, domain, parentId);

		let page: ConfluencePage;
		if (existingPage) {
			page = await this.updatePage(existingPage.id, title, confluenceContent, existingPage.version.number, auth, domain, parentId);
		} else {
			page = await this.createPage(title, confluenceContent, auth, domain, parentId);
		}
		...
		if (mermaidBlocks.length > 0 || imageAttachments.length > 0) {
			...
			if (mermaidBlocks.length > 0) {
				...
				for (const block of mermaidBlocks) {
					try {
						...
						await this.uploadAttachment(page.id, block.filename, svgData, auth, domain);
						...
						const placeholderPattern = new RegExp(`(<p>)?!${block.filename.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}(</p>)?`, 'g');
						updatedContent = updatedContent.replace(placeholderPattern, attachmentMacro);
					} catch (error) {
						console.error(`Error processing mermaid block ${block.id}:`, error);
						new Notice(`Warning: Failed to process Mermaid diagram: ${error.message}`);
			...
			if (imageAttachments.length > 0) {
				new Notice(`Processing ${imageAttachments.length} image attachment(s)...`);

				for (const attachment of imageAttachments) {
					try {
						// Read image file from attachments folder
						const { data, contentType } = await this.readImageAttachment(attachment.filename);

						// Upload as attachment
						await this.uploadBinaryAttachment(page.id, attachment.filename, data, contentType, auth, domain);

						// Replace placeholder with attachment reference (handle potential <p> tags)
						const attachmentMacro = `<ac:image ac:width="500"><ri:attachment ri:filename="${attachment.filename}" /></ac:image>`;
						const placeholderPattern = new RegExp(`(<p>)?${attachment.placeholder}(</p>)?`, 'g');
						updatedContent = updatedContent.replace(placeholderPattern, attachmentMacro);
					} catch (error) {
						console.error(`Error processing image attachment ${attachment.filename}:`, error);
						new Notice(`Warning: Failed to process image ${attachment.filename}: ${error.message}`);
			...
			await this.updatePage(page.id, title, updatedContent, page.version.number, auth, domain, parentId);
		}

		return page;
	}

	async ensureFolderHierarchy(folderPath: string[], auth: string, domain: string): Promise<string | null> {
		let currentParentId: string | null = null;

		for (const folderName of folderPath) {
			let existingPage: ConfluencePage | null = await this.findExistingPage(folderName, auth, domain, currentParentId);
			...
			if (existingPage) {
				...
				const anyPage: ConfluencePage | null = await this.findExistingPage(folderName, auth, domain, null);
				...
				if (anyPage) {
					...
					const archivedPage: ConfluencePage | null = await this.findArchivedPage(folderName, auth, domain);
	...
	async findExistingPage(title: string, auth: string, domain: string, parentId: string | null = null): Promise<ConfluencePage | null> {
		...
		const activePages = data.results.filter((page: any) => page.status !== 'archived');
	...
	async findArchivedPage(title: string, auth: string, domain: string): Promise<ConfluencePage | null> {
	...
	async getPageAncestors(pageId: string, auth: string, domain: string): Promise<ConfluencePage[]> {
	...
	async createPage(title: string, content: string, auth: string, domain: string, parentId: string | null = null): Promise<ConfluencePage> {
		...
		try {
			...
			if (response.status !== 200 && response.status !== 201) {
				...
				throw new Error(`Failed to create page "${title}": ${response.status} - ${errorDetail}`);
			}
	...
	async updatePage(pageId: string, title: string, content: string, currentVersion: number, auth: string, domain: string, parentId: string | null = null): Promise<ConfluencePage> {
		...
		try {
			...
			if (response.status !== 200) {
				const errorDetail = response.json?.message || response.text;
				throw new Error(`Failed to update page "${title}": ${response.status} - ${errorDetail}`);
			}
	...
	createMultipartBody(boundary: string, filename: string, fileContent: string): ArrayBuffer {
	...
	async uploadAttachment(pageId: string, filename: string, data: ArrayBuffer, auth: string, domain: string): Promise<string> {
	...
	async readImageAttachment(filename: string): Promise<{ data: ArrayBuffer; contentType: string }> {
		...
		try {
			...
			throw new Error(`Failed to read attachment ${filename}: ${error.message}`);
		}
	}

	createMultipartBodyBinary(boundary: string, filename: string, fileData: ArrayBuffer, contentType: string): ArrayBuffer {
		const encoder = new TextEncoder();
		const parts: Uint8Array[] = [];
	...
	async uploadBinaryAttachment(pageId: string, filename: string, data: ArrayBuffer, contentType: string, auth: string, domain: string): Promise<string> {
		// Use PUT to create or update attachment (upsert)
		const url = `${domain}/wiki/rest/api/content/${pageId}/child/attachment`;

		// Create multipart form data with binary content
		const boundary = '----WebKitFormBoundary' + Math.random().toString(36).substring(2);
		const body = this.createMultipartBodyBinary(boundary, filename, data, contentType);
		...
		if (response.status !== 200) {
			throw new Error(`Failed to upload binary attachment: ${response.status} - ${response.text}`);
		}
	...
	private convertDataviewJsTags(text: string): string {
		...
		return text.replace(/```dataviewjs\s*dv\.view\([^,]+,\s*\{\s*tags:\s*\[([^\]]+)\][^\}]*\}\s*\)\s*```/g, (_match, tagsStr) => {
			const tags = tagsStr.split(',').map((tag: string) => {
	...
	private convertWikiLinks(text: string): string {
		...
		return text.replace(/\[\[([^\]|]+)(?:\|([^\]]+))?\]\]/g, (_match, path, displayText) => {
	...
	private convertBlockquotes(text: string): string {
		let result = text.replace(/^> (.+)$/gm, '<blockquote><p>$1</p></blockquote>');
	...
	private convertLists(text: string): string {
		...
		for (let i = 0; i < listLines.length; i++) {
			...
			if (ulMatch || olMatch) {
				...
				const currentListType: 'ul' | 'ol' = ulMatch ? 'ul' : 'ol';
	...
	private convertParagraphs(text: string): string {
		...
		const addTrailParagraphs = (input: string) => {
			const parts = input.split(/(<ac:structured-macro[\s\S]*?<\/ac:structured-macro>)/g);
		...
		result = result.replace(/<p>(<(?:h\d|ul|ol|table|blockquote|ac:|hr))/g, '$1');
		result = result.replace(/(<\/(?:h\d|ul|ol|table|blockquote|ac:structured-macro|hr\/)>)<\/p>/g, '$1');
...
class ConfluenceSyncSettingTab extends PluginSettingTab {
	...
	display(): void {
		...
		new Setting(containerEl)
			...
			.setDesc('Your Confluence domain (e.g., https://yourcompany.atlassian.net)')
			.addText(text => text
		...
		new Setting(containerEl)
			...
			.setDesc('Folder path relative to vault root where attachments are stored (e.g., attachments)')
			.addText(text => text
		...
		new Setting(containerEl)
			...
			.setDesc('Automatically add the Confluence page URL to the file frontmatter after syncing')
